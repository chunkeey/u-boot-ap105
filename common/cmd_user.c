...
void test_lan(void)
{
	...
	if (miiphy_info (devname, 0, &oui, &model, &rev) == 0){
		puts ("\n\tLan port checking passed\n\n");
	} else {
		puts ("\n\tLan port checking failed\n\n");
	}


}

int do_port_test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{	
	if (argc < 2){
		printf("%s\n",cmdtp->help);
		return -1;
	...
	if (!strcmp( "gpio", argv[1])){		
		//puts("Press the reset or wps button to see LED lighting\n");	
		puts("Press the reset button to see LED lighting\n");	
		test_gpio();
	} else if (!strcmp("lan", argv[1])){
		test_lan();
	}
...
void reset_phy_ar8021(void)
{
	unsigned short reg_ctrl = 0;	
	ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x00, &reg_ctrl);
	reg_ctrl |=  0x1<<15;	
	ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x00, reg_ctrl);
	udelay(10);	
	
}

int do_giga_mode(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	unsigned int mode = 0;
	unsigned short reg_giga_ctrl = 0;
	...
	ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);
	//printf("reg giga mode %x\n", reg_giga_ctrl);
	switch (mode) {
		case 0:
			reg_giga_ctrl &= ~GIGA_NORMAL;
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, reg_giga_ctrl);			
			ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);
			printf("reg giga mode should be normal %x\n", reg_giga_ctrl);			
			printf("switch giga port to normal mode\n");			
			reset_phy_ar8021();				
			vsc8601_phy_setup(0);
			//ag7100_set_mac_from_link(0, 100, 1);
			return 0;
		case 1:					
			reg_giga_ctrl &= ~GIGA_NORMAL;
			reg_giga_ctrl |= GIGA_TEST_1;
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, reg_giga_ctrl);			
			ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);			
			printf("reg giga mode should be 1 %x\n", reg_giga_ctrl);			
			printf("switch giga port to test mode 1\n");			
			return 0;
		case 2:			
			reg_giga_ctrl &= ~GIGA_NORMAL;
			reg_giga_ctrl |= GIGA_TEST_2;
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, reg_giga_ctrl);			
			ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);			
			printf("reg giga mode should be 2 %x\n", reg_giga_ctrl);			
			printf("switch giga port to test mode 2\n");			
			return 0;
		case 3:				
			reg_giga_ctrl &= ~GIGA_NORMAL;
			reg_giga_ctrl |= GIGA_TEST_3;
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, reg_giga_ctrl);			
			ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);			
			printf("reg giga mode should be 3 %x\n", reg_giga_ctrl);			
			printf("switch giga port to test mode 3\n");			
			return 0;
		case 4:					
			reg_giga_ctrl &= ~GIGA_NORMAL;
			reg_giga_ctrl |= GIGA_TEST_4;
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, reg_giga_ctrl);			
			ag7100_miiphy_read("eth0", CFG_PHY_ADDR, 0x09, &reg_giga_ctrl);			
			printf("reg giga mode should be 4 %x\n", reg_giga_ctrl);			
			printf("switch giga port to test mode 4\n");			
			return 0;		
		case 5:		
			/*disable 1000M autonigotiation*/
			printf("Disable 1000M Auto-Negotiation\n");			
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x1d, 0x000b);
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x1e, 0x0009);				
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x00, 0x8140);			
			ag7100_miiphy_write("eth0", CFG_PHY_ADDR, 0x09, 0x2200);
			break;
		default:
			printf("unsupported mode number\n");
			return -1;
	}
	return 0;
